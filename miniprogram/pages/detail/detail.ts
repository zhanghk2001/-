...
Page({
  ...
  getTicket() {
    ...
    if (app.globalData.status != "user") {
      ...
      wx.cloud.database().collection("experience").doc(id).get().then(res => {
        console.log(res.data.title);
        name = res.data.title
        wx.cloud.database().collection("exticket").add({
          data: {
            ticketid: id,
            title: name,
            isused: false
          ...
          wx.cloud.database().collection("experience").doc(id).get()
            .then(res => {
              ...
              console.log(income);
              wx.cloud.database().collection("experience").doc(id).update({
                data: {
                  countincome: income
                }
              }).then(res => {
