...
Page({
  ...
  getlist(){
    ...
    wx.cloud.database().collection("spotticket").where({
  ...
  clockin(e: any) {
    ...
    var that = this
    wx.cloud.database().collection("spot").doc(e.currentTarget.dataset.id)
      .get().then(res => {
        console.log(res.data.countpeople);
        people = res.data.countpeople
        wx.cloud.database().collection("spot").doc(e.currentTarget.dataset.id).update({
          data: {
            countpeople: people + 1
          }
        }).then(res=>{
          console.log(res);
          wx.cloud.database().collection("spotticket").doc(e.currentTarget.dataset.ticketid).update({
            data: {
              isused:true
            }
          }).then(res=>{
